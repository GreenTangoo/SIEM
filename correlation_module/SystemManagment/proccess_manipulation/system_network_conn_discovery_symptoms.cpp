...
SystemNetworkConnDiscoverySymptoms::SystemNetworkConnDiscoverySymptoms(std::string filename) : json_filename(filename)
{
    this->type_symp = category::symptom_category::system_network_connection_discovery;
}
...
bool SystemNetworkConnDiscoverySymptoms::checkSymptoms()
{
    ...
    for(; container->down != nullptr; container->down = container->down->next)
    {
        ...
        if(((process_name == "netstat") || (process_name == "lsof") || (process_name == "w")) &&
